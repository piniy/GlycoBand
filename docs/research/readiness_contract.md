...
| B — Source and storage | Exact sources, access, license, sizes, checksums, and storage budget verified | Data preparation owner | `IN_PROGRESS` | `configs/data_sources.yaml`; `data/manifests/source_manifest.json` | Source inventory and calculated storage reserve are not frozen | Project lead if access or storage changes scope |
| C — Raw-data audits | Hb-PPG and BIG IDEAs audits complete with exclusions and support evidence | Audit owner | `NOT_STARTED` | `reports/audits/hbppg_audit.md`; `reports/audits/bigideas_audit.md` | Gate B must pass | Independent review |
