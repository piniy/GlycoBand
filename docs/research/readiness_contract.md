...
| B — Source and storage | Exact sources, access, license, sizes, checksums, and storage budget verified | Data preparation owner | `PASS` | `configs/data_sources.yaml`; `data/manifests/source_manifest.json`; `docs/research/source_verification.md` | None; full BIG IDEAs acquisition remains a downstream action | Project lead authorized required downloads on 2026-08-15 |
| C — Raw-data audits | Hb-PPG and BIG IDEAs audits complete with exclusions and support evidence | Audit owner | `IN_PROGRESS` | Hb: `reports/audits/hbppg_audit.md`; BIG: pending | BIG IDEAs acquisition, extracted-file verification, and full audit remain | Independent review |
